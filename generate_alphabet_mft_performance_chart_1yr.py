"""
Generate 4-panel non-overlapping 1-year performance chart for Alphabet MFT program.

Creates a PDF with 4 vertically stacked panels showing:
1. Mean monthly return (1-year non-overlapping)
2. Standard deviation (1-year non-overlapping)
3. CAGR (1-year non-overlapping)
4. Maximum drawdown compounded (1-year non-overlapping)
...
def main():
    """
    Generate non-overlapping 1-year performance chart for Alphabet MFT.
    """
    db = Database('pnlrg.db')
    db.connect()

    print("=" * 70)
    print("ALPHABET MFT - NON-OVERLAPPING 1-YEAR PERFORMANCE CHART")
    print("=" * 70)
    ...
    # Generate non-overlapping 1-year windows working backwards from end date
    print("\nGenerating non-overlapping 1-year windows (reverse from latest date)...")

    # Calculate windows manually (1 year = 12 months)
    windows = []
    current_end = latest_date
    window_num = 1

    while True:
        window_start = date(current_end.year - 1, current_end.month, 1)
        ...
        # Move to next window (1 year earlier, minus 1 day to avoid overlap)
    ...
    for i, win in enumerate(windows):
        ...
        if len(window_data) < 10:  # Need at least 10 months out of 12
        ...
        # CAGR (for 1 year, this is just the cumulative return)
    ...
    # Create a string version of dates for x-axis
    # For 1-year windows, use just the year for cleaner labels
    df['date_label'] = df['date'].dt.strftime('%Y')

    print(f"\nWindow end dates (years): {df['date_label'].tolist()}")
    ...
    fig.update_layout(
        title=dict(
            text=f'<b>{manager_name}: {program_nice_name}</b><br><sub>1-Year Non-Overlapping Performance</sub>',
            font=dict(
    ...
    print(f"  Setting tick labels for {len(tick_vals)} windows")
    ...
    for row in range(1, 5):
        fig.update_xaxes(
            ...
            ticktext=tick_text,
            tickfont=dict(
                size=8,  # Slightly larger since we have year labels only
                family=fonts['family'],
                color=colors.get('text_secondary', '#7f8c8d')
            ),
            tickangle=-45,
            title_text="Period End Year" if row == 4 else "",
            title_font=dict(size=fonts['axis_title_size']),
    ...
    output_path_pdf = "export/alphabet_mft_1yr_performance.pdf"
    output_path_html = "export/alphabet_mft_1yr_performance_debug.html"
    ...
    for idx, row in df.iterrows():
        ...
        print(f"    CAGR (1yr):          {row['prog_cagr']*100:>8.2f}%")
        print(f"    Max Drawdown (1yr):  {row['prog_max_dd']*100:>8.2f}%")
